...
- **Base de datos:** además de `negocios`, `sucursales`, `usuarios_sucursales` e `inventario_sucursal`, ya están operativos `negocio_configuracion`, `inventario_negocio`, `producto_sucursal` y `servicio_sucursal`. La migración 09 refuerza la unicidad de productos/servicios por negocio (`sku`/`nombre`) para que el catálogo global sea consistente y la capa por sucursal maneje personalizaciones (precio, visibilidad, stock mínimo).
- **Backend:** el `ScopedSupabaseClient` mantiene el aislamiento por negocio y ahora existen endpoints para mantener sucursales (`POST/PATCH /businesses/{business_id}/branches`) y para consultar/crear productos en función del modo seleccionado. `stock.get_productos` decide entre `inventario_negocio` y `inventario_sucursal`/`producto_sucursal`, y `BranchSettingsService.update` deja un flag en `metadata` para orquestar el script de sincronización cuando cambia `inventario_modo`.
- **Frontend:** `BusinessContext` expone `branchSettings` y se creó la pantalla de preferencias (`/settings/branch-preferences`) con toggles para inventario/servicios/catálogo, además del formulario de alta de sucursales. El sidebar vuelve a habilitar Configuración y usa las nuevas APIs para refrescar sucursales y preferencias.
- **Operación actual:** las transferencias de stock usan `StockTransferService` con soporte para ambos modos (centralizado o sucursal). El flujo de productos respeta los índices únicos por negocio y, al cambiar de modo, se debe ejecutar `scripts/simulate_inventory_mode_switch.py` para validar la migración de inventario antes de habilitar el feature flag en producción.
...
**Actualizacion 03-11-2025:** Se corrigieron los scripts migration_08a/migration_08b para envolver los `RAISE NOTICE` en bloques `DO $$` y soportar ejecucion via `execute_sql_file.py`. Durante la corrida en local surgio un error por ausencia de `inventario_sucursal`; se documento en scripts/MIGRATION_README.md que las migraciones 01-07 deben ejecutarse previamente. Ademas, se agregaron pruebas unitarias en tests/services/test_branch_settings_service.py para cubrir `BranchSettingsService.fetch/update`, confirmando que crea registros por defecto y respeta metadatos/actualizacion de timestamps.
**Actualizacion 03-11-2025 (tarde):** Backend expone `POST /businesses/{business_id}/branches` y `PATCH /businesses/{business_id}/branches/{branch_id}` (ref. app/api/api_v1/endpoints/businesses.py) con pruebas asociadas en tests/api/test_business_branches.py. En frontend se habilitó la pantalla `BranchPreferences` (`/settings/branch-preferences`) con toggles para inventario/servicios/catálogo y formulario de alta de sucursales, consumiendo `BusinessContext.refreshBranchSettings/refreshBranches`. El sidebar vuelve a habilitar la sección Configuración.
**Actualizacion 03-11-2025 (nochet):** Se creó migration_09_enforce_catalog_uniqueness.sql para blindar unicidad de `productos` y `servicios` por negocio (índices parciales sobre sku/nombre) manteniendo `producto_sucursal` como capa de personalización; la migración se ejecutó localmente con éxito (`python scripts/execute_sql_file.py scripts/migration_09_enforce_catalog_uniqueness.sql`).
**Actualizacion 03-11-2025 (inventory-modes):** Los endpoints de stock (`app/api/api_v1/endpoints/stock.py`) ahora consumen el modo configurado: `GET /businesses/{id}/stock` compone la respuesta con `inventario_negocio` + `vista vw_inventario_visible` cuando el inventario es centralizado y con `inventario_sucursal`/`producto_sucursal` cuando es por sucursal (requiere sucursal activa). Las altas/actualizaciones de productos sincronizan `inventario_negocio` o `inventario_sucursal` según corresponda y replican precios/stock mínimo en `producto_sucursal`. `BranchSettingsService.update` marca `metadata.inventory_mode_sync_required = true` al cambiar el modo para disparar `scripts/simulate_inventory_mode_switch.py`.
